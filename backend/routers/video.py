  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .drop-zone {
      ...
      border-radius: 8px; padding: 36px 20px;
...
<main>
  ...
  <div class="tab-panel active" id="tab-webcam">
    <div class="card">
      ...
      <div class="cam-row">
        <label>Source</label>
        <select id="cam-sel"><option>Detecting cameras...</option></select>
    ...
    <div class="card">
      ...
      <div class="meta-row" id="wc-meta" style="margin-top:16px; display:none">
        <div class="meta-item">
          <span class="meta-key">Frames</span>
          <span class="meta-val" id="wc-frames">—</span>
        </div>
        <div class="meta-item">
          <span class="meta-key">P(fake)</span>
          <span class="meta-val" id="wc-pfake">—</span>
        </div>
        <div class="meta-item">
          <span class="meta-key">Segments</span>
          <span class="meta-val" id="wc-segs">—</span>
        </div>
      </div>
      ...
      <p class="tip">
        <strong>Tip:</strong> For deepfake filter testing, select OBS Virtual Camera with DeepFaceLive running.
      </p>
    </div>
  </div>
  ...
  <div class="tab-panel" id="tab-upload">
    ...
    <div class="card" id="up-result-card" style="display:none">
      ...
      <div class="meta-row" id="up-meta" style="display:none; margin-top:16px">
        <div class="meta-item">
          <span class="meta-key">Segments</span>
          <span class="meta-val" id="up-segs">—</span>
        </div>
        <div class="meta-item">
          <span class="meta-key">Frames</span>
          <span class="meta-val" id="up-frames">—</span>
        </div>
        <div class="meta-item">
          <span class="meta-key">Face visible</span>
          <span class="meta-val" id="up-face">—</span>
        </div>
        <div class="meta-item">
          <span class="meta-key">P(fake)</span>
          <span class="meta-val" id="up-pfake">—</span>
        </div>
      </div>
      ...
      <div class="seg-section" id="seg-section" style="display:none">
        ...
        <table class="seg-table">
          <thead>
            <tr>
              <th>#</th><th>Time</th><th>Verdict</th><th>Confidence</th><th>P(fake)</th>
            </tr>
          </thead>
...
<script>
  ...
  async function startCapture() {
    ...
    try {
      ...
      setVerdict('wc', 'Camera Error', e.message, 'UNCERTAIN');
      return;
    }
  ...
  // ── Polling — exponential backoff, 1.5s start → 6s cap ────────────────────
  // Reduced from old 2s start → 8s cap. Faster first check, tighter ceiling.
  async function pollJob(jobId) {
    ...
    for (let i = 0; i < MAX_POLLS; i++) {
      ...
      interval = Math.min(Math.floor(interval * FACTOR), MAX_DELAY);
      ...
      if (job.status === 'queued') {
        ...
        setVerdict('up', 'Processing', 'Analysing rPPG signal...', 'idle');
      }
  ...
  async function analyzeVideo() {
    ...
    try {
      ...
      if (!resp.ok) {
        setVerdict('up', 'Error', data.detail || resp.statusText, 'UNCERTAIN');
        return;
      }
      setVerdict('up', 'Queued', 'Waiting for worker...', 'idle');
